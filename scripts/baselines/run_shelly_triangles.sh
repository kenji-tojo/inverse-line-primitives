#!/usr/bin/env bash
#
# Train and evaluate every Shelly scene with the triangle-primitive baseline.
#
# Requires band-sampled seed point clouds at
#   datasets/fuzzy_dataset/coarse/neus2_shelly_dtu_15000steps/shelly_<scene>/band_samples/band0.100_n666667.ply
# fetched from the Fuzzy dataset below, before any training starts.
#
# Usage:
#   bash scripts/baselines/run_shelly_triangles.sh                 # results/<default>_<timestamp>/
#   bash scripts/baselines/run_shelly_triangles.sh my_run_name     # results/my_run_name/
#   bash scripts/baselines/run_shelly_triangles.sh my_run_name 0    # torch GPU index (default 0)
#
# Output:
#   results/<run>/
...
ITERS=50000
CKPT_ITERS="7000 50000"
EVAL_ITERS=(50000)
# 666_667 triangles x 3 verts = 2_000_001 vertices.
TARGET_NUM_VERTS=2000001
GPU_ID="${2:-0}"

RUN_NAME="${1:-shelly_triangles_$(date +%Y%m%d_%H%M%S)}"
...
if [ -d "${OUT_DIR}" ] && [ -n "$(ls -A "${OUT_DIR}" 2>/dev/null)" ]; then
    ...
    echo "       Pass a different run name: bash scripts/baselines/run_shelly_triangles.sh <run_name>"
    exit 1
fi
mkdir -p "${OUT_DIR}"

echo "============================================================"
echo "Run: ${RUN_NAME}  (triangle primitives)"
echo "Output: ${OUT_DIR}"
echo "Scenes: ${SCENES[*]}"
echo "Target verts: ${TARGET_NUM_VERTS}"
echo "GPU: torch cuda:${GPU_ID} (CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset: all GPUs visible>}, FUZZYDR_DEVICE_INDEX=${FUZZYDR_DEVICE_INDEX:-<auto>})"
echo "============================================================"
...
for scene in "${SCENES[@]}"; do
    ...
    python scripts/baselines/train_shelly_triangles.py \
        --scene "${scene}" \
        --device cuda --gpu_id "${GPU_ID}" \
        --iters "${ITERS}" \
        --target_num_verts "${TARGET_NUM_VERTS}" \
        --spatial_lr_scale \
    ...
    for eval_it in "${EVAL_ITERS[@]}"; do
        ...
        python scripts/baselines/eval.py \
            --ckpt "${CKPT}" \
            --device cuda --gpu_id "${GPU_ID}" \
            --eval_dir "${EVAL_DIR}"
    done
...
# ------------------------------------------------------------------
# Collect summary: timing + metrics per scene into one JSON
# ------------------------------------------------------------------
echo ""
