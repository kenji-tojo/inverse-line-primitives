#!/usr/bin/env bash
#
# Train and evaluate every Shelly scene with the no-opacity ablation of the
# differentiable rendering formulation: line opacity is fixed to 1 and never
# optimized, so there is no opacity-mask loss and no opacity-based pruning.
#
# Requires band-sampled seed point clouds at
#   datasets/fuzzy_dataset/coarse/neus2_shelly_dtu_15000steps/shelly_<scene>/band_samples/band0.100_n1000000.ply
# fetched from the Fuzzy dataset below, before any training starts.
#
# Usage:
#   bash scripts/baselines/run_shelly_no_opacity.sh                 # results/shelly_no_opacity_<timestamp>/
#   bash scripts/baselines/run_shelly_no_opacity.sh my_run_name     # results/my_run_name/
#
# Output:
#   results/<run>/
...
RUN_NAME="${1:-shelly_no_opacity_$(date +%Y%m%d_%H%M%S)}"
...
if [ -d "${OUT_DIR}" ] && [ -n "$(ls -A "${OUT_DIR}" 2>/dev/null)" ]; then
    ...
    echo "       Pass a different run name: bash scripts/baselines/run_shelly_no_opacity.sh <run_name>"
    exit 1
fi
mkdir -p "${OUT_DIR}"

echo "============================================================"
echo "Run: ${RUN_NAME}  (no-opacity ablation)"
echo "Output: ${OUT_DIR}"
echo "Scenes: ${SCENES[*]}"
...
for scene in "${SCENES[@]}"; do
    ...
    python scripts/baselines/train_shelly_no_opacity.py \
        --scene "${scene}" \
