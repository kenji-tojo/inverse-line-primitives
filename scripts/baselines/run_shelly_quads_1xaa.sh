#!/usr/bin/env bash
#
# Train and evaluate every Shelly scene with the quads_1xAA renderer ablation:
# camera-facing quads of fixed radius, rasterized at native resolution with
...
#
# Usage:
#   bash scripts/baselines/run_shelly_quads_1xaa.sh                 # results/shelly_quads_1xaa_<timestamp>/
#   bash scripts/baselines/run_shelly_quads_1xaa.sh my_run_name     # results/my_run_name/
#
# Output:
#   results/<run>/
...
RUN_NAME="${1:-shelly_quads_1xaa_$(date +%Y%m%d_%H%M%S)}"
...
if [ -d "${OUT_DIR}" ] && [ -n "$(ls -A "${OUT_DIR}" 2>/dev/null)" ]; then
    ...
    echo "       Pass a different run name: bash scripts/baselines/run_shelly_quads_1xaa.sh <run_name>"
    exit 1
fi
mkdir -p "${OUT_DIR}"

echo "============================================================"
echo "Run: ${RUN_NAME}  (quads_1xAA renderer ablation)"
echo "Output: ${OUT_DIR}"
echo "Scenes: ${SCENES[*]}"
...
for scene in "${SCENES[@]}"; do
    ...
    python scripts/baselines/train_shelly_quads_1xaa.py \
        --scene "${scene}" \
