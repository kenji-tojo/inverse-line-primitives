#!/usr/bin/env python3
"""MCMC_reloc baseline: relocation instead of edge splitting.

Topology ablation of the line-primitive method. Pruning is unchanged, with the
same threshold ramp, but the vertex budget freed by pruning is refilled by
relocating fresh lines rather than by splitting the longest edges. Endpoint
snapping is dropped. Everything else is identical.

Relocation rule, for each line needed to refill the budget:

  1. Sample a parent edge from the survivors, p(edge) ~ opacity * length.
  2. Sample t ~ U[0, 1] along that edge.
  3. seed = (1-t)*v_a + t*v_b + N(0, sigma^2 I), sigma = JITTER * init_line_length.
  4. Draw a direction uniformly on the unit sphere.
  5. Emit a line of length init_line_length centred on the seed.

Spherical harmonics are interpolated from the parent at t; opacity is inherited
from the parent's logit. Relocated vertices start with zeroed optimizer state.

Usage:
    python scripts/baselines/train_shelly_mcmc_reloc.py --scene khady
...
# Relocation jitter, as a fraction of init_line_length.
RELOC_JITTER = 0.1
...
def main() -> None:
    ...
    ap.add_argument("--prune_thresh", type=float, default=0.05,
                    help="Opacity threshold: lines with line_opacity < this are "
                         "pruned at each remesh step.")
    ...
    ap.add_argument("--out_dir", type=str, default="results/shelly_mcmc_reloc")
    ...
    print(f"[init] init_line_length={init_line_length:.6f} (mean KNN)")
    ...
    def rebuild_opt():
        ...
        opt_other = torch.optim.Adam(
            sh_groups + [{"params": [line_opacity_logit], "lr": float(args.lr_lop)}],
            foreach=False,
        )
        ...
        for it in pbar:
            ...
            if it >= remesh_start and (it % remesh_every == 0) and it <= remesh_end:
                with torch.no_grad():
                    ...
                    old_sh_dc = sh_dc
                    old_sh_rest = sh_rest
                    old_line_opacity_logit = line_opacity_logit
                    ...
                    # Prune: keep lines whose opacity reaches the threshold.
                    lop_prob = torch.sigmoid(line_opacity_logit.detach())
                    surv = (lop_prob >= cur_prune).nonzero(as_tuple=False).squeeze(-1)
                    K = int(surv.shape[0])

                    # Each line owns two vertices, so the budget is in lines.
                    n_reloc = max(0, cur_target_v // 2 - K)

                    lines_surv = lines_cur.long()[surv]
                    v0, v1 = lines_surv[:, 0], lines_surv[:, 1]
                    # Interleaved: surviving line j owns new vertices 2j, 2j+1.
                    interleaved = torch.stack([v0, v1], dim=1).reshape(-1)

                    verts_new = verts.detach()[interleaved]
                    sh_dc_new = sh_dc.detach()[interleaved]
                    sh_rest_new = sh_rest.detach()[interleaved] if sh_rest is not None else None
                    lop_new = line_opacity_logit.detach()[surv]
                    origin = interleaved

                    if n_reloc > 0 and K > 0:
                        pos = verts.detach()
                        edge_len = (pos[v0] - pos[v1]).norm(dim=-1).clamp_min(1e-12)
                        weights = (lop_prob[surv] * edge_len).clamp_min(1e-20)
                        parent = torch.multinomial(weights, n_reloc, replacement=True)

                        t_par = torch.rand(n_reloc, device=device)
                        seed = ((1.0 - t_par).unsqueeze(-1) * pos[v0[parent]]
                                + t_par.unsqueeze(-1) * pos[v1[parent]])
                        seed = seed + torch.randn_like(seed) * (RELOC_JITTER * init_line_length)

                        rdir = torch.randn(n_reloc, 3, device=device)
                        rdir = rdir / rdir.norm(dim=-1, keepdim=True).clamp_min(1e-8)
                        half = 0.5 * init_line_length
                        reloc_v = torch.stack([seed - rdir * half,
                                               seed + rdir * half], dim=1).reshape(-1, 3)

                        def _interp(p):
                            return ((1.0 - t_par).view(-1, 1, 1) * p[v0[parent]]
                                    + t_par.view(-1, 1, 1) * p[v1[parent]])

                        sh_dc_r = _interp(sh_dc.detach())
                        reloc_sh_dc = torch.stack([sh_dc_r, sh_dc_r], dim=1).reshape(-1, 3, num_sh_dc)
                        verts_new = torch.cat([verts_new, reloc_v], dim=0)
                        sh_dc_new = torch.cat([sh_dc_new, reloc_sh_dc], dim=0)
                        if sh_rest is not None:
                            sh_rest_r = _interp(sh_rest.detach())
                            reloc_sh_rest = torch.stack(
                                [sh_rest_r, sh_rest_r], dim=1).reshape(-1, 3, num_sh_rest)
                            sh_rest_new = torch.cat([sh_rest_new, reloc_sh_rest], dim=0)
                        lop_new = torch.cat([lop_new, lop_new[parent]], dim=0)
                        # Relocated vertices carry no history.
                        origin = torch.cat(
                            [interleaved,
                             torch.full((n_reloc * 2,), -1, dtype=torch.int64, device=device)],
                            dim=0)

                    n_lines_new = K + max(0, n_reloc)
                    base = torch.arange(n_lines_new, device=device, dtype=torch.int64) * 2
                    lines_cur = torch.stack([base, base + 1], dim=1).to(torch.uint32).contiguous()

                    N1 = int(verts_new.shape[0])
                    Ml1 = int(lines_cur.shape[0])
                    old_idx_t = origin

                    verts = torch.nn.Parameter(verts_new.contiguous().requires_grad_(True))
                    sh_dc = torch.nn.Parameter(sh_dc_new.contiguous().requires_grad_(True))
                    if sh_rest is not None:
                        sh_rest = torch.nn.Parameter(sh_rest_new.contiguous().requires_grad_(True))
                    line_opacity_logit = torch.nn.Parameter(lop_new.contiguous().requires_grad_(True))
                    ...
                    replace_tensor_to_optimizer(opt_other, old_line_opacity_logit, line_opacity_logit)

                    needs_topology_update = True

                    pbar.write(
                        f"[reloc] it={it}  lines: {Ml0}->{Ml1}  "
                ...
                with torch.no_grad():
                    ...
                    for ti in range(num_test_views):
                        ...
                        pred = fuzzydr.msaa_downsample_rgba(fuzzydr.rasterize(
                            ...
                            lines=lines_cur, line_opacity=torch.sigmoid(line_opacity_logit),
