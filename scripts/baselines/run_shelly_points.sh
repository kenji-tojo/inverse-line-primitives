#!/usr/bin/env bash
#
# Train and evaluate every Shelly scene with the point-primitive baseline.
#
# Requires band-sampled seed point clouds at
#   datasets/fuzzy_dataset/coarse/neus2_shelly_dtu_15000steps/shelly_<scene>/band_samples/band0.100_n2000000.ply
# fetched from the Fuzzy dataset below, before any training starts.
#
# Usage:
#   bash scripts/baselines/run_shelly_points.sh                 # results/<default>_<timestamp>/
#   bash scripts/baselines/run_shelly_points.sh my_run_name     # results/my_run_name/
#   bash scripts/baselines/run_shelly_points.sh my_run_name 0    # torch GPU index (default 0)
#
# Output:
#   results/<run>/
...
RUN_NAME="${1:-shelly_points_$(date +%Y%m%d_%H%M%S)}"
...
if [ -d "${OUT_DIR}" ] && [ -n "$(ls -A "${OUT_DIR}" 2>/dev/null)" ]; then
    ...
    echo "       Pass a different run name: bash scripts/baselines/run_shelly_points.sh <run_name>"
    exit 1
fi
mkdir -p "${OUT_DIR}"

echo "============================================================"
echo "Run: ${RUN_NAME}  (point primitives)"
echo "Output: ${OUT_DIR}"
echo "Scenes: ${SCENES[*]}"
echo "Point primitives: ${NUM_POINTS}"
...
for scene in "${SCENES[@]}"; do
    ...
    python scripts/baselines/train_shelly_points.py \
        --scene "${scene}" \
