#!/usr/bin/env bash
#
# Train and evaluate every Shelly scene with the MCMC_reloc topology ablation:
# the vertex budget freed by pruning is refilled by relocating fresh lines
# instead of splitting the longest edges.
#
...
#
# Usage:
#   bash scripts/baselines/run_shelly_mcmc_reloc.sh                 # results/shelly_mcmc_reloc_<timestamp>/
#   bash scripts/baselines/run_shelly_mcmc_reloc.sh my_run_name     # results/my_run_name/
#
# Output:
#   results/<run>/
...
RUN_NAME="${1:-shelly_mcmc_reloc_$(date +%Y%m%d_%H%M%S)}"
...
if [ -d "${OUT_DIR}" ] && [ -n "$(ls -A "${OUT_DIR}" 2>/dev/null)" ]; then
    ...
    echo "       Pass a different run name: bash scripts/baselines/run_shelly_mcmc_reloc.sh <run_name>"
    exit 1
fi
mkdir -p "${OUT_DIR}"

echo "============================================================"
echo "Run: ${RUN_NAME}  (MCMC_reloc topology ablation)"
echo "Output: ${OUT_DIR}"
echo "Scenes: ${SCENES[*]}"
...
for scene in "${SCENES[@]}"; do
    ...
    python scripts/baselines/train_shelly_mcmc_reloc.py \
        --scene "${scene}" \
