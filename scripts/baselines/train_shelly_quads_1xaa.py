#!/usr/bin/env python3
"""quads_1xAA baseline: camera-facing quads rendered at native resolution.

Renderer ablation of the line-primitive method. Three differences, everything
else identical:

  1. Lines render as camera-facing quads of fixed world-space radius
     (LINE_RADIUS), not 1-px Bresenham segments. The radius is not optimized.
  2. Rasterization happens at native (W, H) instead of 2x supersampled.
  3. Anti-aliasing is an isotropic Gaussian blur with sigma in output pixels,
     matching the footprint of the 2x MSAA reconstruction filter.

Usage:
    python scripts/baselines/train_shelly_quads_1xaa.py --scene khady

Run every scene:
    bash scripts/baselines/run_shelly_quads_1xaa.sh
...
# World-space quad radius, in world units (5e-4 = 1 mm diameter).  Unlike the
# main method, this value IS the rendered line width: the quads are sized by
# it.  It is held fixed; no radius parameter is optimized.
LINE_RADIUS = 5e-4

# Gaussian anti-aliasing width, in output pixels.
AA_SIGMA = 0.5

_AA_KERNEL_CACHE: dict = {}


def _gaussian_kernel(sigma: float, device, dtype=torch.float32) -> torch.Tensor:
    """Depthwise Gaussian kernel [3, 1, K, K], cached.  K captures +-2 sigma."""
    key = (float(sigma), str(device), dtype)
    if key not in _AA_KERNEL_CACHE:
        ks = max(3, 2 * int(math.ceil(2.0 * float(sigma))) + 1)
        half = ks // 2
        x = torch.arange(-half, half + 1, dtype=dtype, device=device)
        g = torch.exp(-x * x / (2.0 * float(sigma) * float(sigma)))
        g = g / g.sum()
        k2 = g[:, None] * g[None, :]
        _AA_KERNEL_CACHE[key] = k2[None, None].expand(3, 1, ks, ks).contiguous()
    return _AA_KERNEL_CACHE[key]


def gaussian_aa(img_rgba: torch.Tensor) -> torch.Tensor:
    """Isotropic Gaussian AA on [H, W, 4] float32, returning [H, W, 3]."""
    kernel = _gaussian_kernel(AA_SIGMA, img_rgba.device, img_rgba.dtype)
    ks = kernel.shape[-1]
    rgb = img_rgba[..., :3].permute(2, 0, 1).unsqueeze(0).contiguous()
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", type=str, default="results/shelly_quads_1xaa")
    ...
    try:
        ...
        for it in pbar:
            ...
            # Rendering is at native resolution, so the rasterizer mask and
            # the error map already share a size: no upsample here.
