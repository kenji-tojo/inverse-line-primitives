#!/usr/bin/env python3
"""Line-primitive training on the Shelly scenes.

Optimizes line primitives against posed multi-view images.  Every
`remesh_every` iterations discrete_updates.relining prunes lines by opacity
and by length, then splits the longest edges to refill a fixed vertex budget.

Usage:
    python scripts/train_shelly_lines.py --scene khady
    python scripts/train_shelly_lines.py --scene khady --iters 50000 --ckpt_iters 7000 50000
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", type=str, default="results/shelly_lines")
